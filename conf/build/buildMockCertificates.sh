...
openssl req -x509 -new -key testCert/ca.key --days 1024 -out testCert/ca.pem -section ca -config cert_conf/buildOpenssl.conf
...
create_certificate()
{
  ...
  openssl req -x509 -days 365 -noenc -out $DESTINATION.pem -keyout $DESTINATION.key -section $SECTION -config cert_conf/buildOpenssl.conf -CA testCert/ca.pem -CAkey testCert/ca.key
